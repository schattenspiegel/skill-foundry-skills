#!/usr/bin/env python3
"""Compare two Excel workbooks semantically and emit a JSON preservation report."""
...
from inspect_workbook import DEFAULT_DETAILS_LIMIT, SCHEMA_VERSION, inspect
...
def compare(before: dict[str, Any], after: dict[str, Any], limit: int) -> dict[str, Any]:
    ...
    categories = {
        "package": sum(item["path"].startswith("package.") for item in differences),
        "workbook": sum(item["path"].startswith("workbook.") for item in differences),
        "preservation": sum(item["path"].startswith("preservation.") for item in differences),
        "other": sum(
            not item["path"].startswith(("package.", "workbook.", "preservation."))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.details_limit < 0:
        ...
        return 2
    try:
        before = inspect(args.before.resolve(), limit=args.details_limit, include_values=True)
        after = inspect(args.after.resolve(), limit=args.details_limit, include_values=True)
    except Exception as exc:  # keep the CLI's parse-failure contract deterministic
        print(
            json.dumps(
                {
                    "schema_version": SCHEMA_VERSION,
